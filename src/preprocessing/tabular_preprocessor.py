...
class TabularPreprocessor(BasePreprocessor):
    ...
    def transform(self, X: pd.DataFrame, augment: bool = False) -> np.ndarray:
        ...
        for col in self.numeric_cols:
            if col in X_clean.columns:
                # In transform, we realistically should use fit medians, but for simplicity here we just use 0 
                # if completely missing, or we can assume it's pre-filled. Better: fillna(0) for missing test data
